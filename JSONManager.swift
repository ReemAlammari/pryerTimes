...
//  Athan
//
//  Created by Reem Alammari on 02/05/1445 AH.
//
...
class AthanInfo: ObservableObject{
  
    @Published var pryerTimes : Athan?
   // @Published var currentPryerTime: Athan?
    var currentDate  = Date.now.formatted(date: .abbreviated , time: .omitted)
  
    
    func fetch(){
        let JSONData = JsonAthan.data(using: .utf8)!
        
        do{
             pryerTimes = try JSONDecoder().decode(Athan.self, from: JSONData)
              //print(pryerTimes)
//            var  currentPryerTime = pryerTimes?.data.first(where: {$0.date.readable == currentDate})?.timings.Fajr
        
        }catch{
            print("Error Deconding1 : \(error)")
        }
        
        
    }
    
    func getTimePryer()-> Timings? {
        
        return pryerTimes?.data.first(where: {$0.date.readable == currentDate})?.timings
    }
...
//==================================
struct Athan: Decodable  {
    
    var data: [DataAthan]
}

//==================================
struct DataAthan: Decodable {
    var timings: Timings
    var date: DateCatigory
